...
def test_score_source_match_prioritizes_transaction_fact_for_debit_credit_questions() -> None:
    transaction_score = _score_source_match(
        "total debits vs credits by account",
...
def test_score_source_match_prioritizes_teller_authorization_table() -> None:
    teller_score = _score_source_match(
        "pending authorization transactions in teller",
...
def test_score_source_match_prioritizes_term_deposit_maturity_table() -> None:
    td_score = _score_source_match(
        "next deposit contract to mature",
...
def test_resolve_scoped_objects_uses_domain_terms_without_name_error() -> None:
    matches = FakeAnalyticsService().resolve_scoped_objects("failed ATM withdrawals")
...
def test_resolve_scoped_objects_uses_preferred_domain_tables() -> None:
    ...
    cases = [
        ("pending teller transactions", "FLEX_DETB_RTL_TELLER_1"),
        ("next deposit contract to mature", "FLEX_ICTM_TD_DETAILS"),
        ("rejected checks in clearing", "FLEX_CSTB_CLEARING_MASTER"),
        ("business date by branch", "FLEX_STTM_DATES"),
        ("interest by account", "FLEX_ICTB_ACC_PR"),
        ("current balance by currency and branch", "FLEX_STTM_CUST_ACCOUNT"),
        ("accounts with the highest blocked balance", "FLEX_STTM_CUST_ACCOUNT"),
        ("loans with the highest pending debt", "FLEX_CLTC_ACCOUNT_MASTER"),
        ("hidden transactions on statements", "FLEX_EXT_ACCOUNT_STATEMENT"),
        ("users authorized accounting movements", "FLEX_ACTB_DAILY_LOG_1"),
        ("transaction volume by product", "FLEX_EXT_ACCOUNT_TRANSACTIONS"),
    ]
