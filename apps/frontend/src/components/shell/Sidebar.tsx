...
function Icon({
  ...
  kind: 'home' | 'new-chat' | 'search' | 'table' | 'dashboards' | 'settings' | 'users';
}) {
  if (kind === 'new-chat') {
    return (
...
export function Sidebar({
  ...
  const { recentConversations, recentConversationsError, recentConversationsLoading } = sidebarChats;
  ...
  const renderMenuButton = (item: MenuItem) => {
    if ('adminOnly' in item && item.adminOnly && user?.groupId !== 0) return null;
